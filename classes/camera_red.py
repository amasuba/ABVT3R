#!/usr/bin/env python3
"""
Kinect v2 Camera Host - Camera B (Red)
...
class KinectV2CameraHost:
    """
    Kinect v2 RGB-D camera host — Camera B (Red).
    ...
    # None → second enumerated Kinect v2 device (index 1); first if only one
    # device is connected.
    DEFAULT_SERIAL = None
    ...
    def init_camera(self) -> bool:
        ...
        if serial is None or serial not in serials:
            # Prefer index 1 (second device) for Camera B; fall back to 0
            idx = 1 if len(serials) > 1 else 0
            serial = serials[idx]
            print(f"Using Kinect serial: {serial} (device index {idx})")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Kinect v2 Camera Host (Camera B - Red)")
    parser.add_argument(
...
def main():
    ...
    print("=== Kinect v2 Camera Host (Camera B - Red) ===")
